...
/**
 * Servlet implementation class Liste_cmds_validees
 */
@WebServlet(name="Liste_cmds_validees", urlPatterns="/admin/Liste_cmds_validees")
public class Liste_cmds_validees extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Liste_cmds_validees() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Commande> cmds = new CommandeDAO().getAllCmdsValidees();
		request.setAttribute("cmds", cmds);
		
		if(request.getParameter("exp")!=null) {
			int cmd_id = Integer.parseInt(request.getParameter("exp"));
			Commande cmd = new CommandeDAO().getById(cmd_id);
			new CommandeDAO().updateToCmdExpediee(cmd);
			
			System.out.println("UPDATE COMMANDE VALIDEE OK");
			response.sendRedirect("Liste_cmds_validees");
			...
			System.out.println("DELETE COMMANDE VALIDEE OK");	
			response.sendRedirect("Liste_cmds_validees");
		}else {
			request.getRequestDispatcher("liste_cmds_validees.jsp").forward(request, response);			
		}	
